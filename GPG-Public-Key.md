This GPG key is used to sign binary artifacts from the Sliver Authors specifically, as others are welcome to modify and redistribute the project under the terms of the GPLv3 license, an artifact that is not signed by this specific key is not necessarily illegitimate or malicious it simply means it's not from the original authors.  
