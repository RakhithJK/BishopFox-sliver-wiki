...
3. __Perfect Forward Secrecy__: We do get some forward secrecy since the session keys are stored in memory and only the public key encrypted version is sent over the wire recovery of the hardcoded implant keys (implant ECC private key, implant ECC public key, CA ECC public key) should not result in recovery of the session key. Only if the server's private key is compromised is forward secrecy broken. However, it my be desirable to do a full DH-exchange to proected against recovery of the server certificates.