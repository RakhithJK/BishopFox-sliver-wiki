...
1. __No implant-to-server authentication__: One of the nice properties of mutual TLS is that the implant authenticates itself to the server in addition to the server authenticating itself to the implant. In this model the implant never proves to the server it was generated by this server. Since the implant does ECC keys known to the server in the future we plan to have the implant sign the AES session key or some type of nonce during the key setup.
